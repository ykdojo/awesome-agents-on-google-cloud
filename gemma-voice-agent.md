...
The one slow part of this stack turned out to be session storage, the Agent Engine store that keeps each conversation's history. While building the app, I measured the session operations behind every turn. Creating a session, reading it back, and appending new messages each took seconds, and the first session created after a server process started took 10+ seconds. Session I/O, not the GPU, was where the time went.
