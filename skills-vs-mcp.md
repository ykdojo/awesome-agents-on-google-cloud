...
The right question is the one you face for each capability you add to an agent: do I write a skill, or set up an MCP server? This post is a decision guide: a short primer on each, five situations ending in verdicts, and a decision table. Spoiler: they solve different problems, and mature setups use both.
