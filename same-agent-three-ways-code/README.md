...
Hacker News dataset, built on three runtimes. Each leg is self-contained and
...
python leg1_interactions.py "What do people on HN think about Rust?"
```

| File | What it shows |
|---|---|
| `leg1_interactions.py` | Interactions API: hand-written execute_sql tool, you run the loop, server keeps state |
| `leg2_adk.py` | ADK: the first-party BigQuery toolset, framework runs the loop, agent discovers schema |
| `leg3_antigravity.py` | Antigravity SDK: prepackaged harness, BigQuery via Google's hosted MCP endpoint |
| `leg3_container.sh` | Runs leg 3 in a Linux container (no google-antigravity wheel for Intel macOS) |
| `_run_with_gcloud_creds.py` | Test-only ADC shim for this machine, not article material |

Test status (2026-08-06):

- Leg 1: VERIFIED end to end on the Gemini API free tier (`gemini-3.6-flash`);
  strong answer with real quotes and usernames.
- Leg 2: VERIFIED on Gemini Enterprise Agent Platform, the platform formerly
  ...
  routes around it.
- Leg 3: VERIFIED end to end on GEAP + the hosted BigQuery MCP endpoint
  ...
  `Dockerfile.leg3` bakes the image (`leg3-antigravity`) for fast starts.
