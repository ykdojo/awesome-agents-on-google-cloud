...
The pipeline turns the finished script into speech by splitting it into segments of roughly 90 seconds, calling the TTS model once per segment, and concatenating the audio at the end. A single call for the whole episode goes through, but it does not really work: the audio quality degrades after the first few minutes. Each segment call looks like this:
